 ...
 * | **Nested JWE** | 5 segments, `cty: "JWT"` header | Decryption -> inner JWS signature verification. |
 *
 ...
 * 5. Run registered [Validator] beans (optional).
 * 6. Return [AuthData].
 *
 * @see group.phorus.auth.commons.services.Authenticator
 */
@AutoConfiguration
@Service
@Primary
class StandaloneAuthenticator(
     ...
     * Per [RFC 7519 §5.2](https://datatracker.ietf.org/doc/html/rfc7519#section-5.2),
    ...
    private fun validateEncryptionConfig() {
        ...
        requireNotNull(enc.encodedPrivateKey) {
            "group.phorus.security.jwt.encryption.encoded-private-key must be set for token format ${securityConfiguration.jwt.tokenFormat}"
        }
    ...
    private fun validateSigningConfig() {
        ...
        requireNotNull(signing.encodedPublicKey) {
            "group.phorus.security.jwt.signing.encoded-public-key must be set for token format ${securityConfiguration.jwt.tokenFormat}"
        }
