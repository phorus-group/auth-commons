 ...
 * [SecurityConfiguration.jwt.tokenFormat][group.phorus.auth.commons.config.JwtConfiguration.tokenFormat].
 *
 * ### Supported formats
 *
 ...
 * | [TokenFormat.NESTED_JWE] | A JWS wrapped inside a JWE (sign-then-encrypt). Provides **both** integrity and confidentiality. The outer JWE header contains `cty: "JWT"` per [RFC 7519 §5.2](https://datatracker.ietf.org/doc/html/rfc7519#section-5.2). |
 *
 * In [IDP_DELEGATED][AuthMode.IDP_DELEGATED] mode, this bean is still registered but
 * key validation is skipped at startup (the bean's methods should not be called in that mode).
 *
 * @see group.phorus.auth.commons.services.TokenFactory
 */
@AutoConfiguration
@Service
class TokenFactoryImpl(
    ...
    private val tokenFormat: TokenFormat get() = securityConfiguration.jwt.tokenFormat

    init {
        // Only validate keys when the service will actually create tokens
        if (securityConfiguration.mode != AuthMode.IDP_DELEGATED) {
            val format = securityConfiguration.jwt.tokenFormat
    ...
    override suspend fun createAccessToken(
        ...
        val expiration = currentTime.plusSeconds(securityConfiguration.jwt.expiration.tokenMinutes * 60)
    ...
    private fun buildJwe(claimsMap: Map<String, Any>, tokenType: TokenType): String {
        ...
        val enc = securityConfiguration.jwt.encryption
     ...
     *
     * The outer JWE header includes `cty: "JWT"` per RFC 7519 §5.2 to indicate that the
    ...
    private fun buildClaimsMap(
        ...
        securityConfiguration.jwt.issuer?.let { put("iss", it) }
    ...
    private fun resolveSigningPrivateKey(): PrivateKey {
        val signing = securityConfiguration.jwt.signing
    ...
    private fun resolveSignatureAlgorithm() =
        securityConfiguration.jwt.signing.signatureAlgorithm?.let { algId ->
    ...
    private fun validateEncryptionConfig() {
        ...
        requireNotNull(enc.encodedPublicKey) {
            "group.phorus.security.jwt.encryption.encoded-public-key must be set for token format $tokenFormat"
        }

        require(Security.getAlgorithms("KeyFactory").any { it.equals(enc.algorithm, ignoreCase = true) }) {
            "group.phorus.security.jwt.encryption.algorithm '${enc.algorithm}' not found. Available: ${Security.getAlgorithms("KeyFactory")}"
        }

        require(Jwts.KEY.get().contains(enc.keyAlgorithm)) {
            "group.phorus.security.jwt.encryption.key-algorithm '${enc.keyAlgorithm}' not found. Available: ${Jwts.KEY.get().keys}"
        }

        require(Jwts.ENC.get().contains(enc.aeadAlgorithm)) {
            "group.phorus.security.jwt.encryption.aead-algorithm '${enc.aeadAlgorithm}' not found. Available: ${Jwts.ENC.get().keys}"
        }
    }
    ...
    private fun validateSigningConfig() {
        ...
        requireNotNull(signing.encodedPrivateKey) {
            "group.phorus.security.jwt.signing.encoded-private-key must be set for token format $tokenFormat"
        }

        require(Security.getAlgorithms("KeyFactory").any { it.equals(signing.algorithm, ignoreCase = true) }) {
            "group.phorus.security.jwt.signing.algorithm '${signing.algorithm}' not found. Available: ${Security.getAlgorithms("KeyFactory")}"
        }

        signing.signatureAlgorithm?.let { algId ->
            require(Jwts.SIG.get().contains(algId)) {
                "group.phorus.security.jwt.signing.signature-algorithm '$algId' not found. Available: ${Jwts.SIG.get().keys}"
            }
        }
    }
}
