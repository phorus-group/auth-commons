 ...
 * that OpenSSL produces), creates a token with [TokenFactoryImpl], and parses it
 * back with [StandaloneAuthenticator]. This proves the algorithm + key-format combination
...
class TokenFactoryImplTest {
    ...
    companion object {
        ...
        private fun roundTrip(config: SecurityConfiguration) = runBlocking {
            val factory = TokenFactoryImpl(config)
            val authenticator = StandaloneAuthenticator(config, emptyList())
    ...
    inner class EcEncryption {
        private val sigKeys = ecKeyPair("secp384r1")
